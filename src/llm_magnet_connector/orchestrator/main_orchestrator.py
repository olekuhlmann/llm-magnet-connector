...
from llm_magnet_connector.llm_interface import LLMConversationManager, LLMResponse, get_reprompt
...
class MainOrchestrator:
    ...
    def __init__(self, llm_manager: LLMConversationManager, image_generator: ResponseToImage, max_iterations: int):
        """
    ...
    def run(self, initial_prompt: str, initial_images_dir: str):
        ...
        while not self.is_terminated(response):
            ...
            print(f"Re-prompting for iteration {self._iteration}.")
            prompt = get_reprompt(response.optimizer_parameters, self._image_generator.image_index)
    ...
    def is_terminated(self, response: LLMResponse) -> bool:
        ...
        """
        terminated = False
        if all(not x for x in response.badness_criteria): # all False
            terminated = True
        return terminated
